#!/usr/bin/env bash
# header-lint.sh — lmmi-b-studio
#
# Guards the family binary-accent header extraction (PR #14).
# Runs against every tracked *.html file at repo root.
#
# Check 1 — Inline stripe regression
#   Fail if a <style> block contains a CSS rule for .stripe AND
...
#   Fail if the file uses <div class="stripe"> AND does not link
...
for file in "${HTML_FILES[@]}"; do
  ...
  # ------------------------------------------------------------------
  # Check 1: .stripe in inline <style> with repeating-linear-gradient
  # ------------------------------------------------------------------
...
PYEOF
  ...
  # ------------------------------------------------------------------
  # Check 2: file uses <div class="stripe"> without /assets/header.css
  # ------------------------------------------------------------------
  if grep -qE 'class="[^"]*stripe[^"]*"|class='"'"'[^'"'"']*stripe[^'"'"']*'"'" "$file"; then
    if ! grep -q 'href="/assets/header.css"' "$file"; then
      echo "[header-lint] FAIL Check 2 ($rel): uses <div class=\"stripe\"> but missing <link rel=\"stylesheet\" href=\"/assets/header.css\">."
      ERRORS=$((ERRORS + 1))
    fi
  fi
